...
bpRegionEnrichment <- function(bedDir='inst/extdata/bed', breakpoints=NA, slop=0, plot=TRUE, genome_length=118274340 ){
...
writeBed <- function(svs, bedFile){
  svs <- svs %>% 
    mutate(info = paste(sample, type, feature, feature2, sep = "_")) %>% 
    select(chrom, bp, bp2, info)
  
  write.table(svs, file = bedFile, row.names=F, col.names=F, sep=" ", quote = FALSE)
}
