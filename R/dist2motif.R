...
plotdistanceOverlay <- function(..., d, feature="tss", lim=10, byChrom=NA, n=10){
  ...
  # p <- p + geom_density(data = combined[combined$Source=='Sim',], aes(min_dist, colour=iteration, fill=iteration), alpha = 0.1, adjust=0.5)
  # p <- p + geom_density(data = combined[combined$Source=='Real',], aes(min_dist, fill = Source), alpha = 0.5, adjust=0.5)
