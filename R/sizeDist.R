...
sizeDist <- function(...) {
  ...
  bp_data <- bp_data %>%
    ...
    add_count(genotype) %>%
    mutate(genotype_short  = fct_recode(genotype, G_r = "germline_recurrent", G_p = "germline_private", S_n = "somatic_normal", S_t = "somatic_tumour"))
...
genotypeTypeCount <- function(...) {
  ...
  p <- ggplot(bp_data)
  p <- p + geom_bar(aes(fct_reorder(genotype_short, freq), freq, fill=type), alpha=0.7, stat = 'identity', position=position_dodge()) 
